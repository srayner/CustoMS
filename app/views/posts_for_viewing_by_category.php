...
<header class="page-header">
	<h1>The Latest News <small>articles</small> in the  <?php if($_SESSION['current_category'] != null){ echo $_SESSION['current_category']; }else { echo "unavailable"; } ?> category</h1>
</header>

<?php

	if(isset($_SESSION['msg']['posts_viewing_by_category-error'])){
		echo '<div class="alert alert-danger" role="alert">' . $_SESSION['msg']['posts_viewing_by_category-error'] . '</div>';
		unset($_SESSION['msg']['posts_viewing_by_category-error']);
	}
...
<section>
	<?php foreach($_SESSION['posts_for_viewing_by_categories_results'] AS $post){ ?>
		<?php
			$id 			= $post['post_id'];
			$title 			= $post['title'];
