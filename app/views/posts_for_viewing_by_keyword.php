...
<header class="page-header">
	<h1>The Latest News <small>articles</small> containing '<?php echo $_SESSION['user_requested_keyword']; ?>'</h1>
</header>

<?php

	if(isset($_SESSION['msg']['posts_for_viewing_by_keyword_results-error'])){
		echo '<div class="alert alert-danger" role="alert">' . $_SESSION['msg']['posts_for_viewing_by_keyword_results-error'] . '</div>';
		unset($_SESSION['msg']['posts_for_viewing_by_keyword_results-error']);
	}

?>

<?php if($_SESSION['posts_for_viewing_by_keyword_results'] != null){ ?>
	<section>
		<?php foreach($_SESSION['posts_for_viewing_by_keyword_results'] AS $post){ ?>
			<?php
				$id 			= $post['post_id'];
				$title 			= $post['title'];
